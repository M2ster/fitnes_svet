...
def book_lesson(
    ...
    try:
        ...
        # Если это абонемент "Полный" - можно на всё, кроме детской йоги
        if "полный" in membership_type_name:
            # Проверяем, что занятие НЕ детская йога
        ...
        # Проверка для детской йоги (можно на любую детскую йогу)
        elif "детская йога" in membership_type_name and "детская йога" in lesson_type_name:
            is_match = True
        # Проверка для взрослой йоги
        elif "взрослая йога" in membership_type_name and "взрослая йога" in lesson_type_name:
            is_match = True
        # Проверка для фитнес-пилатес-йога
        elif any(x in membership_type_name for x in ["фитнес", "пилатес", "йога"]) and \
                any(x in lesson_type_name for x in ["фитнес", "пилатес", "йога"]):
            # Исключаем растяжку и детскую йогу из этой группы
            if "растяжка" not in lesson_type_name and "детская" not in lesson_type_name:
                is_match = True
        # Проверка для растяжки
        elif "растяжка" in membership_type_name and "растяжка" in lesson_type_name:
            is_match = True
        # Проверка для разовых абонементов (они привязаны к конкретному типу)
